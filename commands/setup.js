...
module.exports = {
    ...
    if (DISABLED) {
      return message.reply("Command disabled for further use. Ask the administrator or owners of this bot to add your server until we fix some bugs with this command.")
    }
		if (!args || args.length === 0) return message.reply("no channel given")
		...
		if (!serverCode) {
			message.channel.send(`Servername required.`)
		} else {
